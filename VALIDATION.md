...
环境：Python 3.14.7、`cryptography` 46.0.7、`tkinterdnd2` 0.6.3、Tk 9.0、FFmpeg 8.1.2。本文件不列出用户的本机路径、曲名或音频指纹，以便公开发布。
...
Tk 可创建、刷新并销毁根窗口；`tkdnd` 运行包可加载并注册文件拖放目标；`ncm-restore-gui` 启动后进程保持运行，未崩溃。通过桌面自动化截图检查了 980×720 窗口：蓝色拖放区、输入列表、紧凑选项区、结果列表和固定在右下角的“开始转换”按钮均在首屏可见。此轮没有完成真实鼠标拖放的端到端验收。GUI 的拖放路径处理、格式映射和批量逻辑由自动测试覆盖；正式发布前宜人工拖入并转换一份样例。
